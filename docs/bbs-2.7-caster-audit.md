...
- `BbsModelSilhouetteChecks`: кость позы берётся через `Pose.getOrCreate`. С BBS 2.6 `Pose.get`
  не создаёт отсутствующую кость, и cubic-самотест падал NPE ещё до первой проверки позы
  (сбой только в оснастке, сэмплер не задет; найдено при первом прогоне 2026-09-21).
...
- `caster-revision-checks(cubic): PASS 18` (плоская cubic-модель `mobs/chair` в мире Flat,
  BBS 2.7-1.20.4): 15 проверок линии 2.6 + 3 новых кейса 2.7.
- Выигрыш на статичной сцене (2026-09-21, `-Dirlite.profileVl=true`, мир Flat из `runs/1.20.4`:
  25 villager MobForm-блоков и 25 спотов, Complementary r5.9.3, игрок неподвижен), окна по
  ~120 кадров: на кадр `caster.known 25`, `caster.unknown 1` (игрок, by design), `sp.reuse 25`;
  `gpu bake` p50 0.01 мс; cpu frame p50 8.3 мс, pipeline p50 1.6 мс. Совпадает с §9 спеки 2.5.2.

## 5. Открыто

1. **Линия 1.21.x не аудирована и на 2.7**: `port/1.21.1` получил те же правки (гейт событий,
