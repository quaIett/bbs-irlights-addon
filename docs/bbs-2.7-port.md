...
1. **Редактор `refreshedui` 1.2.0-fs2.6 не грузится на 2.7**: его собственный `UIReplaysEditorMixin`
   ссылается на удалённый `ReplayCategory` и роняет клиент на этапе APPLY. Из `run/mods` он убран в
   `run/mods-disabled` (там же лежат прежние отключённые сборки). Порт редактора — отдельная задача,
   возвращать jar в `run/mods` только по слову пользователя.
2. **Caster-revision bridge не аудирован для 2.7** (на всех версиях): `BbsSilhouetteBridge.audit()`
   знает только `2.6-1.20.1`/`2.6-1.20.4`, поэтому на 2.7 ревизии остаются UNKNOWN и бейк теней идёт полным —
   корректно, но без переиспользования. Дрейф в `cubic` реальный (ModelInstance +49, ModelCube +168,
   ProceduralAnimator, ModelGroup, ModelPivotFrames), поэтому версию нельзя просто дописать в список
   без повторного силуэт-аудита.
...
4. Коммитов нет ни на одной из трёх веток; работа ветки 2.0 лежит в стэше.
