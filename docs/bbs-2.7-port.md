...
- `runClient` на BBS 2.7: 1.20.4, 1.21.11 и 1.21.1 стартуют без единой ошибки миксинов
  ...
  штатные uniform'ы CR (`BIOME_PALE_GARDEN`, `endFlashIntensity`) на 1.21.1.
- Игровая приёмка пользователем 2026-09-21 — PASS на 1.20.4, 1.20.1, 1.21.1 и 1.21.11 (Complementary
  r5.9.3 с текущим патчем): вкладка Light (кнопка, цифровой шорткат, скрытие без ламп), секции
  лампы, Lit/Outline Replays, ключи эффектов, гайды спота.
...
2. **Кастеры**: на master силуэт-аудит 2.7 сделан (`docs/bbs-2.7-caster-audit.md`, мост пишет
   `audited` на 2.7-1.20.1/1.20.4); cubic-самотест (PASS 18) и выигрыш на статичной сцене
   (25 known / 25 reuse на кадр, bake 0.01 мс) подтверждены 2026-09-21. Линия 1.21.x не аудирована намеренно — своего аудита она не
...
4. **Замечено в самом BBS 2.7 (не наш код)**: на 1.21.1 шейдеры `assets/bbs/shaders/core/pixelart_text.vsh`
   и `pixelart_text_intensity.vsh` вызывают `fog_distance(ModelViewMat, pos, FogShape)` в сигнатуре 1.20
   и не компилируются (всплывает при открытии редактора фильма); на 1.21.11 `UIPickableFormRenderer.renderAxes`
   → `UIFormEditor.getOrigin` бросает NPE (`editor == null`) на кадре после закрытия редактора формы —
   BBS сам ловит его и пропускает гизмо на этот кадр.
