...
с копиями редактора и CR+DOF. Новая библиотека
также копируется в локальный runtime CR.

2026-09-15 те же три изменения перенесены в RethinkingVoxels, BSL, Solas, Bliss,
IterationRP и Photon: дельта `reference → CR` влита трёхсторонним слиянием,
в каждом паке она текстуально равна дельте CR, а cookie/shadow helpers совпадают
с CR. Main-патчи перегенерированы (Original + patch == Modification), копии
редактора и шесть CR+DOF-комбо синхронизированы, DOF-хвосты не изменились.
Harness с per-pack prelude для символов пака дал по 330 float-сравнений на пак,
maxAbs 0. Прогон шёл на Intel UHD (гибридный ноутбук), тайминги не показательны;
игровая компиляция в Iris для этих паков не проверялась.

Замена JAR сама по себе не изменяет
